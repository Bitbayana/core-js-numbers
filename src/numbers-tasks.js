...
function getSumOfDigits(num) {
  ...
  // Извлечение цифр числа и добавление к сумме
  while (absNum > 0) {
    sum += absNum % 10;
...
function toPrecision(number, precision) {
  const fixedNotation = number.toPrecision(precision);
  const exponentialNotation = number.toExponential(precision - 1);

  return fixedNotation.length < exponentialNotation.length
    ? fixedNotation
    : exponentialNotation;
}
